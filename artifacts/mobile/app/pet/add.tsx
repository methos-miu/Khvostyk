...
import { CAT_BREEDS_UK, DOG_BREEDS_UK, CAT_BREEDS_EN, DOG_BREEDS_EN } from "@/constants/breeds";
import { Species, Gender, usePets } from "@/context/PetsContext";
import { useLanguage } from "@/context/LanguageContext";
import { toISODate } from "@/utils/notifications";
...
// Weight values: 0.5 to 100 in 0.5 steps
const WEIGHT_VALUES = Array.from({ length: 200 }, (_, i) =>
  ((i + 1) * 0.5).toFixed(1)
);
...
export default function AddPetScreen() {
  ...
  const getBreedList = () => {
    if (form.species === "cat") return language === "uk" ? CAT_BREEDS_UK : CAT_BREEDS_EN;
    if (form.species === "dog") return language === "uk" ? DOG_BREEDS_UK : DOG_BREEDS_EN;
    return [];
  };
  ...
  const handleSave = async () => {
    ...
    const isoDate = toISODate(form.birthdate.trim());
    if (!isoDate) {
      Alert.alert("", t.errorBirthdateFormat);
      return;
    }
    setLoading(true);
    try {
  ...
  const showBreedOption = form.species === "cat" || form.species === "dog";
      ...
      >
        <View style={styles.modalOverlay}>
          <Animated.View entering={FadeInDown.springify()} style={styles.modalSheet}>
            ...
            <FlatList
              data={[...breedList, language === "uk" ? "Інша порода" : "Other breed"]}
              ...
              renderItem={({ item }) => (
                <TouchableOpacity
                  onPress={() => {
                    Haptics.selectionAsync();
                    updateForm("breed", item === (language === "uk" ? "Інша порода" : "Other breed") ? "" : item);
              ...
              <FlatList
                ...
                renderItem={({ item }) => (
                  <Pressable
                    ...
                    style={[
                      styles.weightItem,
                      form.weight === item && styles.weightItemActive,
                    ]}
                  >
                    <Text style={[
...
const styles = StyleSheet.create({
  ...
  weightItemActive: {},
  weightItemText: {
    fontSize: 20, fontFamily: "Inter_400Regular", color: Colors.textSecondary,
  },
  weightItemTextActive: {
    fontSize: 22, fontFamily: "Inter_700Bold", color: Colors.primary,
  },
});
